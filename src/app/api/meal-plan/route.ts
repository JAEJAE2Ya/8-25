...
export async function POST(request: Request) {
  let input: { target?: Record<string, number>; preferences?: Record<string, string> } = {};
  try {
    input = await request.json();
  } catch {
    return NextResponse.json({ mode: "demo", plan: mockMealPlan });
  }

  const key = process.env.OPENAI_API_KEY;
  if (!key) return NextResponse.json({ mode: "demo", plan: mockMealPlan });
  ...
  if (recent.length >= 4) return NextResponse.json({ error: "잠시 후 다시 시도해 주세요." }, { status: 429 });
...
정확히 3일, 매일 breakfast→lunch→dinner 순서로 3끼를 만드세요. 각 끼와 일일 합계는 목표의 약 ±10% 범위에서 현실적으로 맞추세요. 한국 마트에서 쉽게 구하는 재료를 쓰고, 닭고기·달걀·두부·양파·대파·채소처럼 여러 메뉴에 재사용할 수 있는 재료를 의도적으로 배치하세요. 9끼가 지나치게 반복되지는 않아야 합니다. 아침 25%, 점심 35%, 저녁 40% 정도로 열량을 분배하세요. 양은 1인분 기준으로 무리하지 않게 쓰고, 조리법은 짧고 실제로 실행 가능해야 합니다. 의료적 주장이나 치료 표현은 금지합니다. 영양 수치는 추정치로만 다루세요. id는 day1-breakfast 형식으로 고정하세요. emoji는 음식과 어울리는 이모지 하나를 사용하세요.`;
  ...
  try {
    ...
    if (!isValidPlan(plan)) throw new Error("Invalid structured output");
    return NextResponse.json({ mode: "live", plan });
  } catch {
    return NextResponse.json({ mode: "demo", plan: mockMealPlan });
  }
}
