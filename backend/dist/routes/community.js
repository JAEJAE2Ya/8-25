...
const authorSelect = { id: true, nickname: true, avatarUrl: true };
...
async function findPost(db, id, viewerId) {
...
export function registerCommunityRoutes(app, db, config) {
    ...
    app.patch("/api/community/posts/:id", async (request, reply) => {
        const user = await requireAuth(request, reply, db, config);
        if (!user)
            return;
    ...
    app.delete("/api/community/posts/:id", async (request, reply) => {
        const user = await requireAuth(request, reply, db, config);
        if (!user)
            return;
        const owned = await db.communityPost.findFirst({ where: { id: request.params.id, authorId: user.id }, select: { id: true } });
    ...
    app.post("/api/community/posts/:id/like", async (request, reply) => {
        const user = await requireAuth(request, reply, db, config);
        if (!user)
            return;
    ...
    app.delete("/api/community/posts/:id/like", async (request, reply) => {
        const user = await requireAuth(request, reply, db, config);
        if (!user)
            return;
    ...
    app.get("/api/community/posts/:id/comments", async (request, reply) => {
        const post = await db.communityPost.findUnique({ where: { id: request.params.id }, select: { id: true } });
        if (!post)
            return reply.code(404).send({ error: "community_post_not_found" });
        const viewer = await getAuthUser(request, db, config);
    ...
    app.post("/api/community/posts/:id/comments", async (request, reply) => {
        const user = await requireAuth(request, reply, db, config);
        if (!user)
            return;
        const post = await db.communityPost.findUnique({ where: { id: request.params.id }, select: { id: true } });
    ...
    app.delete("/api/community/comments/:id", async (request, reply) => {
        const user = await requireAuth(request, reply, db, config);
        if (!user)
            return;
        const owned = await db.communityComment.findFirst({ where: { id: request.params.id, userId: user.id }, select: { id: true } });
        if (!owned)
            return reply.code(404).send({ error: "community_comment_not_found" });
        await db.communityComment.delete({ where: { id: owned.id } });
        return reply.code(204).send();
    });
    app.post("/api/community/posts/:id/plan", async (request, reply) => {
        const user = await requireAuth(request, reply, db, config);
        if (!user)
            return;
        const body = asObject(request.body);
