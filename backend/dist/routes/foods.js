...
const normalizeFoodName = (name) => name.trim().toLocaleLowerCase("ko-KR");
export function registerFoodRoutes(app, db, config) {
    ...
    app.delete("/api/foods/user/:id", async (request, reply) => {
        const user = await requireAuth(request, reply, db, config);
        if (!user)
            return;
        ...
        if (!owned)
            return reply.code(404).send({ error: "user_food_not_found" });
