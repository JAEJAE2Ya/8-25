...
const favoriteFoodDto = (item: { id: string; foodIdentifier: string; foodName: string; manufacturer: string | null; referenceAmount: number; servingUnit: string; calories: number; carbs: number; protein: number; fat: number; source: string }) => ({
  favoriteId: item.id,
...
export function registerFavoriteRoutes(app: FastifyInstance, db: PrismaClient, config: AppConfig) {
  ...
  app.delete<{ Params: { id: string } }>("/api/favorites/foods/:id", async (request, reply) => {
    const user = await requireAuth(request, reply, db, config);
    if (!user) return;
    const owned = await db.favoriteFood.findFirst({ where: { id: request.params.id, userId: user.id }, select: { id: true } });
    if (!owned) return reply.code(404).send({ error: "favorite_food_not_found" });
  ...
  app.delete<{ Params: { id: string } }>("/api/favorites/recipes/:id", async (request, reply) => {
    const user = await requireAuth(request, reply, db, config);
    if (!user) return;
    const owned = await db.favoriteRecipe.findFirst({ where: { id: request.params.id, userId: user.id }, select: { id: true } });
    if (!owned) return reply.code(404).send({ error: "favorite_recipe_not_found" });
