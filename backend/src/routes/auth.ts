...
const safeUser = (user: { id: string; email: string; nickname: string; avatarUrl: string | null; createdAt: Date }) => ({
  id: user.id,
  email: user.email,
...
export function registerAuthRoutes(app: FastifyInstance, db: PrismaClient, config: AppConfig) {
  app.post("/api/auth/signup", { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (request, reply) => {
    ...
    if (nickname.length < 2) throw new ValidationError("nickname must be at least 2 characters");
